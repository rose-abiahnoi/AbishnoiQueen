...
from config import (QueenKU_API_KEY, QueenKU_APP_NAME, UPSTREAM_BRANCH,
                    UPSTREAM_REPO)
from Queen import LOG_GROUP_ID, MUSIC_BOT_NAME, SUDOERS, app
from Queen.Database import get_active_chats, remove_active_chat, remove_active_video_chat
from Queen.Utilities.Queenku import is_Queenku, user_input
from Queen.Utilities.paste import isPreviewUp, paste_queue
...
/get_log
- Get log of last 100 lines from Queenku.

/get_var
- Get a config var from Queenku or .env.

/del_var
- Delete any var on Queenku or .env.

/set_var [Var Name] [Value]
- Set a Var or Update a Var on Queenku or .env. Seperate Var and its Value with a space.
...
async def log_(client, message):
    if await is_Queenku():
        if QueenKU_API_KEY == "" and QueenKU_APP_NAME == "":
            return await message.reply_text(
                "<b>QueenKU APP DETECTED!</b>\n\nIn order to update your app, you need to set up the `QueenKU_API_KEY` and `QueenKU_APP_NAME` vars respectively!"
            )
        elif QueenKU_API_KEY == "" or QueenKU_APP_NAME == "":
            return await message.reply_text(
                "<b>QueenKU APP DETECTED!</b>\n\n<b>Make sure to add both</b> `QueenKU_API_KEY` **and** `QueenKU_APP_NAME` <b>vars correctly in order to be able to update remotely!</b>"
            )
    else:
        return await message.reply_text("Only for Queenku Apps")
    try:
        Queenku = Queenku3.from_key(QueenKU_API_KEY)
        happ = Queenku.app(QueenKU_APP_NAME)
    except BaseException:
        return await message.reply_text(
            " Please make sure your Queenku API Key, Your App name are configured correctly in the Queenku"
        )
    data = happ.get_log()
    if len(data) > 1024:
        ...
        url = link + "/index.txt"
        return await message.reply_text(
            f"Here is the Log of Your App[{QueenKU_APP_NAME}]\n\n[Click Here to checkout Logs]({url})"
        )
    else:
...
async def varget_(client, message):
    ...
    if await is_Queenku():
        ...
        Queenku_config = happ.config()
        if check_var in Queenku_config:
            return await message.reply_text(
                f"**Queenku Config:**\n\n**{check_var}:** `{Queenku_config[check_var]}`"
            )
        else:
    ...
    if await is_Queenku():
        ...
        if check_var in Queenku_config:
            await message.reply_text(
                f"**Queenku Var Deletion:**\n\n`{check_var}` has been deleted successfully."
            )
            del Queenku_config[check_var]
        else:
...
async def usage_dynos(client, message):
    ...
    dyno = await message.reply_text("Checking Queenku Usage. Please Wait")
    account_id = Queenku.account().id
    useragent = (
    ...
    headers = {
        ...
        "Authorization": f"Bearer {QueenKU_API_KEY}",
        "Accept": "application/vnd.Queenku+json; version=3.account-quotas",
    }
    path = "/accounts/" + account_id + "/actions/get-quota"
    r = requests.get("https://api.Queenku.com" + path, headers=headers)
...
async def update_(client, message):
    ...
    if await is_Queenku():
        try:
            await response.edit(
                f"{nrs.text}\n\nBot was updated successfully on Queenku! Now, wait for 2 - 3 mins until the bot restarts!"
            )
            os.system(
...
async def restart_(_, message):
    ...
    if await is_Queenku():
        if QueenKU_API_KEY == "" and QueenKU_APP_NAME == "":
            return await message.reply_text(
                "<b>QueenKU APP DETECTED!</b>\n\nIn order to restart your app, you need to set up the `QueenKU_API_KEY` and `QueenKU_APP_NAME` vars respectively!"
            )
        elif QueenKU_API_KEY == "" or QueenKU_APP_NAME == "":
            return await message.reply_text(
                "<b>QueenKU APP DETECTED!</b>\n\n<b>Make sure to add both</b> `QueenKU_API_KEY` **and** `QueenKU_APP_NAME` <b>vars correctly in order to be able to restart remotely!</b>"
            )
        try:
            served_chats = []
            try:
            ...
            for x in served_chats:
                try:
                    ...
                    pass
            Queenku3.from_key(QueenKU_API_KEY).apps()[QueenKU_APP_NAME].restart()
            await response.edit(
